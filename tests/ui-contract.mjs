...
for(const id of ['appShell','map','serviceList','serviceCount','modeTabs','mapStyles','detailPanel','fromInput','toInput','planButton','swapButton','plannerStatus','fromSuggestions','toSuggestions','fromLocationButton','toLocationButton','locationStatus'])assert.match(html,new RegExp(`id=["']${id}["']`),`missing #${id}`);
...
assert.match(app,/geometryPlan/,'network and journey geometry must use the shared geometry precedence core');
assert.match(app,/osrmWaypointCoordinates/,'known corridors must provide ordered OSRM waypoints');
assert.match(app,/MAP_STYLES/);
assert.match(app,/tiles\.openfreemap\.org\/styles\/liberty/,'Transit must use a vector network-emphasis style');
assert.match(app,/World_Imagery/,'Satellite must have imagery provider');
assert.match(app,/restoreMapOverlays/,'map-style changes must restore route plan overlays');
assert.match(html,/data-map-style="transit"/);
assert.match(html,/data-map-style="standard"/);
assert.match(html,/data-map-style="satellite"/);

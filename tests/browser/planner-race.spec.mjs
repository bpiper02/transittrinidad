...
async function primeStaticNetwork(page){
...
test('editing an endpoint cancels a slow plan without stranding the Plan button disabled',async({page})=>{
  await primeStaticNetwork(page);

  let markOsrmStarted;
  let releaseOsrm;
  const osrmStarted=new Promise(resolve=>{markOsrmStarted=resolve;});
  const osrmRelease=new Promise(resolve=>{releaseOsrm=resolve;});
  await page.route('https://router.project-osrm.org/route/v1/driving/**',async route=>{
    markOsrmStarted();
    await osrmRelease;
    await route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({routes:[{duration:900,distance:12000,geometry:{coordinates:[[-61.47,10.42],[-61.41,10.51]]}}]})});
  });
  ...
  await page.locator('#fromInput').fill('Couva');
  await expect(page.locator('#planButton')).toBeEnabled();

  releaseOsrm();
  await expect(page.locator('#fromInput')).toHaveValue('Couva');

...
function scheduleHtml(service){
  ...
  if(!schedule)return `<section class="schedule-block"><p class="eyebrow">Schedule</p>${picker}<p>No published PTSC timetable for this date.</p><span class="schedule-badge">Scheduled · not live</span></section>`;
  const days=formatServiceDays(schedule.serviceDays);
  if(schedule.status!=='published_times'||!schedule.departureTimes.length){
    return `<section class="schedule-block"><p class="eyebrow">Schedule</p>${picker}<strong>Runs ${escapeHtml(days)}</strong><p>Exact departure times are unavailable from PTSC.</p><span class="schedule-badge">Scheduled · not live</span></section>`;
  }

...
function routeColor(service){
  if(service.mode==='maxi'){
    ...
    const band = String(service.bandColor||'').toLowerCase();
    const named = {yellow:1,red:2,green:3,black:4,brown:5,blue:6}[band];
...
function serviceFeature(service){
  ...
  return coordinates ? {
    type:'Feature',
    properties:{id:service.id,mode:service.mode,pathKind:displayPathKind(service),routeColor:routeColor(service)},
...
function renderDetail(service){
  ...
  panel.hidden = false;
  panel.innerHTML = `<div class="journey-summary"><div class="route-title-row"><span class="route-swatch large" style="--route-color:${routeColor(service)}"></span><div><p class="eyebrow">${escapeHtml(modeLabel(service.mode))}</p><h2>${escapeHtml(origin?.name)} → ${escapeHtml(destination?.name)}</h2></div></div><p class="service-line">${fare} · ${escapeHtml(displayPathLabel(service))} path</p></div>`;
}
...
async function autocompletePlaces(query,signal){
  ...
  const response=await fetch(`${PHOTON_BASE}?${params}`,{signal,headers:{Accept:'application/json'}});
  if(!response.ok) return [];
  const data=await response.json();
  return (data.features||[]).map(feature=>{
    const [lng,lat]=feature.geometry?.coordinates||[];
    return {name:photonLabel(feature.properties),lat:+lat,lng:+lng};
  }).filter(place=>place.name&&Number.isFinite(place.lat)&&Number.isFinite(place.lng)&&inTT(place.lng,place.lat));
}
...
function setupAutocomplete(inputId,menuId){
  ...
  input.addEventListener('input',()=>{
    ...
    if(query.length<2){ menu.hidden=true; return; }
    timer=setTimeout(async()=>{
      autocompleteControllers.get(inputId)?.abort();
      const controller=new AbortController();
      autocompleteControllers.set(inputId,controller);
      try{ renderSuggestions(inputId,menuId,await autocompletePlaces(query,controller.signal)); }
      catch(error){ if(error.name!=='AbortError') menu.hidden=true; }
    },300);
  });
...
async function resolvePlace(inputId){
  const input=$(`#${inputId}`);
  const known=findNodeByInput(input.value);
...
function journeyGeoJson(from,to,fromNear,toNear,legs){
  ...
  for(const leg of legs){
    const coordinates=serviceCoordinates(leg.service);
    if(coordinates) features.push({type:'Feature',properties:{kind:'transit',routeColor:routeColor(leg.service),pathKind:displayPathKind(leg.service)},geometry:{type:'LineString',coordinates:leg.service.originNodeId===leg.from?coordinates:[...coordinates].reverse()}});
  }
  features.push({type:'Feature',properties:{kind:'access',routeColor:'#8E8E93'},geometry:{type:'LineString',coordinates:[[toNear.node.location.lng,toNear.node.location.lat],[to.lng,to.lat]]}});
...
function showJourneyMap(from,to,fromNear,toNear,legs){
  ...
  const bounds=new maplibregl.LngLatBounds();
  bounds.extend([from.lng,from.lat]); bounds.extend([to.lng,to.lat]);
  for(const leg of legs) for(const point of serviceCoordinates(leg.service)||[]) bounds.extend(point);
...
function renderJourney(from,to,fromNear,toNear,legs){
  ...
  const allFaresKnown=legs.length>0&&legs.every(leg=>Number.isFinite(leg.service.fareTTD));
  const transfers=Math.max(0,legs.length-1);
  let html=`<div class="journey-summary"><p class="eyebrow">Route</p><h2>${escapeHtml($('#fromInput').value)} → ${escapeHtml($('#toInput').value)}</h2><div class="journey-kpis"><div><strong>${transfers}</strong><span>transfer${transfers===1?'':'s'}</span></div><div><strong>${allFaresKnown?`TT$${knownFare}`:'—'}</strong><span>fare</span></div></div></div>`;
  ...
  legs.forEach(leg=>{
    const origin=nodeIndex.get(leg.from),destination=nodeIndex.get(leg.to);
    const fare=Number.isFinite(leg.service.fareTTD)?` · TT$${leg.service.fareTTD}`:'';
    html+=`<div class="journey-leg"><span class="leg-route" style="--route-color:${routeColor(leg.service)}"></span><div><h3>${escapeHtml(origin.name)} → ${escapeHtml(destination.name)}</h3><p>${escapeHtml(modeLabel(leg.service.mode))}${fare}</p></div></div>`;
  });
  ...
  if(legs.some(leg=>displayPathKind(leg.service)!=='verified')) html+=`<p class="route-note">Some paths are estimated.</p>`;
...
function setupPlanner(){
  ...
  $('#swapButton').addEventListener('click',()=>{
    ...
    fromInput.value=toInput.value; toInput.value=fromValue;
  ...
  $('#planButton').addEventListener('click',async()=>{
    const status=$('#plannerStatus'),button=$('#planButton');
    button.disabled=true; status.textContent='Finding route…';
    try{
      const knownFrom=findNodeByInput($('#fromInput').value),knownTo=findNodeByInput($('#toInput').value);
      const from=await resolvePlace('fromInput'),to=await resolvePlace('toInput');
      const connected=chooseConnectedJourney({fromPlace:from,toPlace:to,nodes:nodeIndex,services,knownFrom,knownTo,candidateLimit:6});
      if(!connected){
        const fromNear=knownFrom?{node:knownFrom,km:0}:nearestNode(from),toNear=knownTo?{node:knownTo,km:0}:nearestNode(to);
        if(!fromNear||!toNear) throw new Error('No nearby transport coverage yet.');
        showJourneyMap(from,to,fromNear,toNear,[]);
        status.textContent='No connected route in the current network.';
        return;
      }
      ...
      status.textContent=`${legs.length} leg${legs.length===1?'':'s'} · via ${fromNear.node.name}`;
...
function addMapLayers(){
  ...
  map.addLayer({id:'service-lines',type:'line',source:'services',layout:{'line-join':'round','line-cap':'round'},paint:{
    'line-color':['get','routeColor'],'line-width':['case',['==',['get','pathKind'],'verified'],3,2.5],'line-opacity':['case',['==',['get','pathKind'],'connector'],.18,.42],
  ...
  map.on('click','service-lines',event=>{ const id=event.features?.[0]?.properties?.id; if(id) selectService(id,false); });
}
...
async function start(){
  try{
    ...
    map=new maplibregl.Map({
      ...
      bounds:TT_BOUNDS,fitBoundsOptions:{padding:50},maxBounds:TT_MAX_BOUNDS,minZoom:7,maxZoom:17,attributionControl:true
    });
    map.addControl(new maplibregl.NavigationControl({showCompass:false}),'bottom-right');
    map.on('load',()=>{ addMapLayers(); fitCountry(); hydrateDisplayGeometry().catch(error=>console.warn('Background route estimation failed',error)); });
